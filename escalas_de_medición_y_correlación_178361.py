# -*- coding: utf-8 -*-
"""Escalas de medición y correlación 178361.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1SrKal1lsno8m0ZsNmPg2-1wO5znKExtb
...
anno = np.array([2020,2021,2022,2023,2024])
precio_s = np.array([89990, 91990, 93990, 95990,97990])
precio_x = np.array([99990, 101990, 103990, 105990,107990])
precio_y = np.array([44990, 46990, 48990, 50990,52990])

def regresion_lineal_model_s():
  # Calcular la media de
  anno_mean = np.mean(anno)
  precio_s_mean = np.mean(precio_s)

  # Calcular la pendiente (m)
  numerator = np.sum((anno - anno_mean) * (precio_s - precio_s_mean))
  denominator = np.sum((anno - anno_mean)**2)
  m = numerator / denominator

  # Calcular la ordenada al origen (b)
  b = precio_s_mean - m * anno_mean

  print(f"Pendiente (m): {m}")
  print(f"Ordenada al origen (b): {b}")

  # Calcular la matriz de correlación
  correlation_matrix = np.corrcoef(anno, precio_s)
...
def regresion_lineal_model_x():
  # Calcular la media de
  anno_mean = np.mean(anno)
  precio_x_mean = np.mean(precio_x)

  # Calcular la pendiente (m)
  numerator = np.sum((anno - anno_mean) * (precio_x - precio_x_mean))
  denominator = np.sum((anno - anno_mean)**2)
  m = numerator / denominator

  # Calcular la ordenada al origen (b)
  b = precio_x_mean - m * anno_mean

  print(f"Pendiente (m): {m}")
  print(f"Ordenada al origen (b): {b}")

  # Calcular la matriz de correlación
  correlation_matrix = np.corrcoef(anno, precio_x)
...
def regresion_lineal_model_y():
  anno_mean = np.mean(anno)
  precio_y_mean = np.mean(precio_y)

  # Calcular la pendiente (m)
  numerator = np.sum((anno - anno_mean) * (precio_y - precio_y_mean))
  denominator = np.sum((anno - anno_mean)**2)
  m = numerator / denominator

  # Calcular la ordenada al origen (b)
  b = precio_y_mean - m * anno_mean

  print(f"Pendiente (m): {m}")
  print(f"Ordenada al origen (b): {b}")

  # Calcular la matriz de correlación
  correlation_matrix = np.corrcoef(anno, precio_y)
